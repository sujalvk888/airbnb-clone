...
// 1. GET ALL LISTINGS (Public Route for Homepage)
...
// MUST BE ABOVE /:id so Express doesn't treat 'my-listings' as an ID parameter
router.get('/my-listings', verifyToken, async (req, res) => {
  try {
    const myListings = await prisma.listing.findMany({
      ...
      include: {
        ...
        bookings: { 
          ...
          select: { totalPrice: true } // Only pull what we need for math
        }
      },
      orderBy: { createdAt: 'desc' }
    });
    ...
    const formatted = myListings.map(listing => {
      ...
      // Calculate total earnings generated by this property
